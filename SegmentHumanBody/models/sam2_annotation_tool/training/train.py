...
def flatten_config(cfg, target_key="scratch"):
    """
    Finds and extracts the first occurrence of `target_key` in an OmegaConf object,
    preserving interpolations and avoiding dictionary conversion.
    """
    def find_target_layer(node):
        if isinstance(node, DictConfig) and target_key in node:
            return node  # Found the target level
        if isinstance(node, DictConfig):
            for key, value in node.items():
                print("Key: ", key)
                if isinstance(value, (OmegaConf, DictConfig)):
                    found = find_target_layer(value)
                    if found:
                        return found
        return None  # Not found

    # Search directly in OmegaConf without conversion
    extracted_cfg = find_target_layer(cfg)

    return extracted_cfg if extracted_cfg else cfg  # Return original if not found
...
def main(args) -> None:
    ...
    print("cfg.launcher.experiment_log_dir", cfg.launcher.experiment_log_dir)
    if cfg.launcher.experiment_log_dir is None:
        cfg.launcher.experiment_log_dir = str(args.config).replace("sam2_annotation_tool/sam2", "sam2_annotation_tool/sam2_logs")
...
if __name__ == "__main__":
    initialize_config_module("models/sam2_annotation_tool/training", version_base="1.2")
