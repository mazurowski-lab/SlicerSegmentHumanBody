...
def load_state_dict_into_model(
    ...
    try:
        ...
        state_dict = update_dict(
            state_dict,
            except_keys=["no_mem_embed", "no_mem_pos_enc", "maskmem_tpos_enc"]
        )
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)

    # try:
    #     model.cond_memory_attention.load_state_dict(model.memory_attention.state_dict())
    #     print("cond_memory_attention: Load memory attention weights successfully!")
    # except:
    #     print("cond_memory_attention: Unsuccessfully initialize memory attention weights!")

    try:
        model.recent_memory_attention.load_state_dict(model.memory_attention.state_dict())
        print("recent_memory_attention: Load memory attention weights successfully!")
    except:
        print("recent_memory_attention: Unsuccessfully initialize memory attention weights!")
...
def update_dict(sd, except_keys=None):
    if except_keys:
        new_sd = {}
        for k, v in sd.items():
            keep_flag = True
